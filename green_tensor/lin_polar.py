...
#  eps[i] = float(input("Диэлектрическая проницаемость материала для слоя: "))
#  #Нормированные радиусы слоев
#  a[i] = float(input("Нормированный радиус для слоея: "))
#  #Магнитная проницаемость материала
#  miy[i] = 1

    #Примеры параметров для оптимизированных моделей ЛЛ
...
class lin_polar:
  ...
  def calc():
    ...
    global E_teta, k0, toch, teta_start, teta_stop, steps, n, eps, a, miy, P_teta
    ...
    return P_teta, E_teta
  
  def print(P_teta, E_teta, steps, teta, n):
    #Conrol Point
    '''
    print('P_teta:', P_teta)
    for z in range((toch)):
  ...
  calc()
  print(P_teta, E_teta, steps, teta, n)
lin_polar()