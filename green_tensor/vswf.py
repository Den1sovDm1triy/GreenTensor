...
# --------------------------------------------------------------------------- #
def wigner_d(n: int, beta: float) -> np.ndarray:
    """Малая d-матрица Вигнера d^n_{m'm}(β), размер (2n+1)×(2n+1), индексы m',m=−n..n
    (строка i'=m'+n, столбец i=m+n). Точная формула Вигнера через факториалы (вещественна).
    """
    f = math.factorial
    ...
    for mp in range(-n, n + 1):
        for m in range(-n, n + 1):
            pref = math.sqrt(f(n + mp) * f(n - mp) * f(n + m) * f(n - m))
            kmin = max(0, m - mp)
            kmax = min(n + m, n - mp)
            s = 0.0
            for k in range(kmin, kmax + 1):
                denom = f(n + m - k) * f(n - mp - k) * f(k + mp - m) * f(k)
                p = 2 * n + m - mp - 2 * k        # степень cos(β/2)
                q = 2 * k + mp - m                 # степень sin(β/2)
                s += (-1) ** (k + mp - m) * (cb ** p) * (sb ** q) / denom
            d[mp + n, m + n] = pref * s
    return d


def wigner_D(n: int, alpha: float, beta: float, gamma: float) -> np.ndarray:
    """Полная D-матрица Вигнера D^n_{m'm}(α,β,γ)=e^{−i m' α} d^n_{m'm}(β) e^{−i m γ}
    (конвенция z-y-z, активный поворот; унитарна)."""
    d = wigner_d(n, beta)
    mvals = np.arange(-n, n + 1)
    left = np.exp(-1j * mvals * alpha)[:, None]
    right = np.exp(-1j * mvals * gamma)[None, :]
    return left * d * right


def rotation_matrix(nmax: int, alpha: float, beta: float, gamma: float) -> np.ndarray:
    """Блочно-диагональная по n матрица вращения (2K×2K) в базисе [блок M; блок N].

    Поворот действует ОДИНАКОВО на M- и N-типы (TE/TM сохраняются), поэтому оба блока
    получают один и тот же блок-диагональный по n набор D^n. Для применения к T-матрице:
    T_повёрнутая = D · T · D^† (см. :func:`rotate_tmatrix`).
    """
    modes = mode_list(nmax)
    K = len(modes)
    Dh = np.zeros((K, K), complex)                 # один полублок (M); N — такой же
    offset = 0
    for n in range(1, nmax + 1):
        Dn = wigner_D(n, alpha, beta, gamma)        # (2n+1)×(2n+1)
        sz = 2 * n + 1
        Dh[offset:offset + sz, offset:offset + sz] = Dn
        offset += sz
    Z = np.zeros((K, K), complex)
    return np.block([[Dh, Z], [Z, Dh]])


def rotate_tmatrix(T: np.ndarray, nmax: int, alpha: float, beta: float, gamma: float) -> np.ndarray:
    """Повернуть T-матрицу (база [M;N], моды mode_list) на углы Эйлера (α,β,γ), z-y-z.

    Тело, заданное в собственной системе (ось симметрии ∥ z), помещается в глобальную с
    ориентацией (α,β,γ): T_global = D(α,β,γ)·T·D(α,β,γ)^†. Для осесимметричного тела
    поворот вокруг своей оси (β=0) оставляет T неизменной; сфера инвариантна к любому
    повороту. См. tests/test_rotation.py.
    """
    D = rotation_matrix(nmax, alpha, beta, gamma)
    return D @ T @ D.conj().T
