...
for i in range(toch):
  nu = i + 1
  # Индекс импеданса: ВНЕШНИЙ слой len(a)-1 (как в lin_polar / 01_sphere).
  # Было Z[i][n-2] — импеданс на внутренней границе при mH/mHpr на внешней
  # (рассогласование радиусов, проверено аналитикой: ошибка до 25x).
  h_out = len(a) - 1
  Mn[i] = (Z[i][h_out] * mJ[i] - mJpr[i]) / (Z[i][h_out] * mH[i] - mHpr[i])
  Mn[i] = Mn[i].real - Mn[i].imag * 1j
  Nn[i] = (Y[i][h_out] * mJ[i] - mJpr[i]) / (Y[i][h_out] * mH[i] - mHpr[i])
