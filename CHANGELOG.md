...
- `green_tensor/mie_reference.py` — эталонная Ми-реализация (downward-recurrence
  ...
  усечения Уискомба.
- `green_tensor/multilayer_mie.py` — многослойная сфера с произвольными
  (ε_l, μ_l) методом лог-производной; PEC-ядро с многослойным покрытием;
  обобщённые формулы Керкера для магнитной сферы; **автоматическая ветка
  произвольной точности** (mpmath, 60 знаков) при |Im(m·k0·r)| > 8 —
  double precision теряет ~1 дБ по Q_back к |Im z| ≈ 18 и разваливается (NaN)
  за ≈ 30.
- `tests/test_multilayer_regression.py` (+ в `run_all.py`): энергобаланс,
  парадокс экстинкции, сходимость Уискомба, сравнение с Бором--Хаффманом и
  Керкером (≤ 1e-10), согласие double/mpmath-веток (≤ 1e-9).
- `examples/Example 7 - MOGA Lens Stratification` — двухкритериальная
  NSGA-II-стратификация линзы Люнеберга с верификацией оптимумом Ллойда
  (RMSE(ε) = 0.0320 при L = 8).
- `examples/Example 8 - RPM Coated PEC Sphere` — РПМ-покрытия ZIK 51-2190/2191
  на проводящей сфере (упражняет mpmath-ветку).

### Fixed
- Версия обёртки `greentensor.py` синхронизирована с пакетом (была 0.2.0).
...
### Fixed
- **GMM, направление вектора трансляции**: в `solve_cluster` вектор передавался
  с обратным знаком; для несимметричных кластеров это нарушало энергобаланс на
  десятки процентов (зеркально-симметричные конфигурации ошибку маскировали).
- **Фазовая конвенция T-матрицы сферы**: ядро `01_sphere` хранит сопряжённые
  коэффициенты Ми (Mn = a_n*), и T-вектор строился без обратного сопряжения —
  нарушалось граничное условие на поверхности и фазы связи тел в кластере.
  Оба дефекта закрыты регрессионными тестами (PEC-граничное условие,
  несимметричный кластер, тождество трансляции).
- GMM: добавлена проверка непересечения описывающих сфер (ValueError вместо
  молчаливо неверного результата); угловая квадратура C_sca масштабируется с
  k·размером кластера; полюсный сдвиг ВСВФ подобран по минимуму ошибки (1e-5).
- Цилиндр: явные ошибки на критическом угле слоя (косое падение) и при
  переполнении цилиндрических функций (|Im(m·x)| > 700).
- Теория: исправлены асимптотики M/N в дальней зоне, Im-форма оптической
  теоремы, конвенция вектора трансляции и примечание о сопряжении в T-матрице.

### Added (верификация)
- Регрессия полной угловой ДН слоистой сферы против Ansys HFSS (МКЭ):
  tests/test_hfss_reference.py + эталоны tests/references/examples.json
  (провенанс в tests/references/README.md).
