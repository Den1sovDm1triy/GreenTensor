...
class RCSCalculator:
    ...
    def calculate_impedances(self):
        """Расчет импедансов и адмитансов на границах раздела сред.

        Z[i][h] и Y[i][h] определены для h = 0..len(a)-1, что соответствует
        пропагации импеданса от внутренней границы r=a[0] до внешней r=a[-1].
        Последняя итерация h = len(a)-1 — пропагация через внешний вакуумный
        слой (от a[-2] до a[-1] = 1), необходимая для корректного матчинга
        с выходящей сферической волной в calculate_scattering_coefficients,
        который теперь читает Z[i][len(a)-1] (а не Z[i][n-2], как раньше).

        Хронология исправлений 2026-05-05:
        - этап 1 (off-by-one): удалены мусорные множители /2 и *2 из бывшей
          elif h==len(a)-1 ветки, плюс убран хак-паддинг alfa/eps в
          run_calculation();
        - этап 2 (multilayer): восстановлена последняя итерация со штатной
          Möbius-формулой пропагации; eps[h+1]/alfa[h+1] для последнего
          шага берутся как eps[h]/alfa[h] (вакуум-вакуум, sqrt_part = 1).
        """
        n_a = len(self.a)
        for i in range(self.toch - 1):
            for h in range(n_a):
                if h == 0:
                    ...
                    # Внешняя для h-й границы среда: при h+1 < n_a — из массивов,
                    # иначе берём текущий слой (последний слой = вакуум, по
                    # соглашению API), что даёт sqrt_part = 1 (без скачка).
                    if h + 1 < n_a:
                        eps_out = self.eps[h + 1]
                        alfa_out = self.alfa[h + 1]
                    else:
                        eps_out = self.eps[h]
                        alfa_out = self.alfa[h]

                    numerator = cmath.exp(alfa_out * 1j) * abs(eps_out)
                    ...
                    denominator = cmath.exp(alfa_out * 1j) * abs(eps_out)
                    sqrt_part = cmath.sqrt(numerator / denominator)
    ...
    def calculate_scattering_coefficients(self):
        ...
        Импеданс/адмитанс берётся в последней (внешней) ячейке, где Z уже
        пропагирован через внешний вакуумный слой к радиусу r = a[-1] = 1.
        Это согласуется с modified-функциями mJ/mJpr/mH/mHpr, которые
        вычислены при k0 (т.е. при том же r = 1).

        До 2026-05-05: использовалось Z[i][n-2], что давало корректный
        результат только в вырожденном случае a[-2] = a[-1] (single-layer);
        для многослойного случая это давало O(1) ошибку. См. парный фикс в
        calculate_impedances().
        """
        h_outer = len(self.a) - 1
        for i in range(self.toch):
            self.Mn[i] = (self.Z[i][h_outer] * self.mJ[i] - self.mJpr[i]) / (self.Z[i][h_outer] * self.mH[i] - self.mHpr[i])
            self.Mn[i] = self.Mn[i].real - self.Mn[i].imag * 1j
            self.Nn[i] = (self.Y[i][h_outer] * self.mJ[i] - self.mJpr[i]) / (self.Y[i][h_outer] * self.mH[i] - self.mHpr[i])
    ...
    def run_calculation(self):
        ...
        # 2026-05-05: убран хак-паддинг alfa/eps мусорным элементом.
        # Раньше здесь стояло `self.eps = np.append(self.eps, len(self.eps))`
        # для обхода IndexError в calculate_impedances(); сам IndexError был
        # следствием off-by-one в цикле по h, который теперь исправлен.
