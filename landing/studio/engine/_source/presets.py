...
# Линейка облучателей csc²-пресета: θ′ с шагом 8° (лучи на элевациях 17°..65°),
# амплитуды стартуют от закона a ∝ csc(элевация) и дотюнены итеративным синтезом
# (6 итераций по точкам лучей; движок studio.compute, ЛЛ 4 слоя, k0=6π, toch=40);
# фазовый прогресс 30°/облучатель сглаживает межлучевую интерференцию.
# Качество огибающей: RMS 0.69 дБ, max |err| 1.3 дБ в секторе элеваций 17..65°.
_CSC_FEEDS = [
    {"offset_deg": 25.0, "amp": 0.237, "phase_deg": 0.0},
    {"offset_deg": 33.0, "amp": 0.266, "phase_deg": 30.0},
    {"offset_deg": 41.0, "amp": 0.232, "phase_deg": 60.0},
    {"offset_deg": 49.0, "amp": 0.382, "phase_deg": 90.0},
    {"offset_deg": 57.0, "amp": 0.639, "phase_deg": 120.0},
    {"offset_deg": 65.0, "amp": 0.247, "phase_deg": 150.0},
    {"offset_deg": 73.0, "amp": 1.0, "phase_deg": 180.0},
]


def _csc_reference():
    """Идеальная csc²-огибающая (дБ) в секторе лучей — накладывается на E-панель ДН.

    Лучи выходят при θ = 360° − θ′ (зеркально облучателям); элевация над экраном
    ε = 90° − θ′. Огибающая нормирована к 0 дБ на нижнем луче (ε_min)."""
    off = [f["offset_deg"] for f in _CSC_FEEDS]
    lo, hi = min(off), max(off)
    e_min = 90.0 - hi
    theta, db = [], []
    t = 360.0 - hi
    while t <= 360.0 - lo + 1e-9:
        elev = 90.0 - (360.0 - t)
        theta.append(round(t, 2))
        db.append(round(20.0 * math.log10(math.sin(math.radians(e_min)) /
                                          math.sin(math.radians(elev))), 3))
        t += 0.5
    return [{"plane": "E", "name": "Идеальная csc²-огибающая",
             "theta_deg": theta, "dB": db}]


def _hemisphere_csc_preset():
    """Косеканс-квадратная ДН на полусферической ЛЛ (линейка из 7 облучателей)."""
    body = _sphere([
        (0.25, 1.94, 0.0), (0.50, 1.75, 0.0), (0.75, 1.44, 0.0), (1.00, 1.00, 0.0),
    ])
    body["hemisphere"] = {"enabled": True, "feeds": [dict(f) for f in _CSC_FEEDS]}
    sc = _scene(body, k=6.0 * _PI, toch=40)
    sc["sweep"]["enable"] = False
    return [{
        "id": "hemisphere_csc",
        "title": "Полусфера: косеканс-квадратная (csc²) ДН — линейка 7 облучателей",
        "group": "Демонстрации",
        "description": "Косеканс-квадратная ДН — классика обзорных РЛС: G(ε) ∝ csc²(ε) даёт "
                       "постоянную мощность эха от цели на фиксированной высоте независимо от "
                       "дальности. Здесь она сформирована на полусферической 4-слойной линзе "
                       "Люнеберга (k0=6π) на PEC-экране ЛИНЕЙКОЙ из 7 точечных облучателей "
                       "(θ′ = 25°..73°, шаг 8°): каждый даёт «карандашный» луч на элевации "
                       "ε = 90°−θ′ (лучи зеркальны облучателям — image theory), а амплитудные "
                       "веса a ∝ csc(ε), дотюненные итеративным синтезом (фазовый прогресс 30° "
                       "сглаживает межлучевую интерференцию), выстраивают огибающую csc² в "
                       "секторе элеваций 17°–65°: RMS отклонения 0.7 дБ, максимум 1.3 дБ. "
                       "Красная пунктирная кривая — идеальная csc²-огибающая для сравнения. "
                       "Облучатели видны на 3D-сцене (размер конуса ∝ амплитуде), их состав "
                       "редактируется в «Свойствах тела»; ДН накладывается на модель "
                       "(чекбокс «ДН»).",
        "scene": sc,
        "reference": _csc_reference(),
    }]
...
# Полусферные пресеты — первыми среди демонстраций.
PRESETS = (_example2_preset() + _examples_presets()
           + _hemisphere_preset() + _hemisphere_csc_preset() + _DEMO_PRESETS)
